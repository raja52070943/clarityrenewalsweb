...
function Dashboard() {
  ...
  const getCobraId = async (companyId) => {
    try {
      const response = await axios.get(`http://localhost:7147/api/Companies/${companyId}`);
      // Extracting cobraPlan.id from the response
      return response.data.cobraPlan ? response.data.cobraPlan.id : null;
    } catch (error) {
      console.error('Error fetching cobraId:', error);
      return null; // Return null if there's an error
    }
  };

  const handleNavigate = async (companyId) => {
    const cobraId = await getCobraId(companyId);
    if (cobraId) {
      navigate(`/Cobra/${cobraId}`);
    } else {
      console.error('Cobra ID not found');
    }
  };
